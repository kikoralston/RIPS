...
def getRenewableCFs(genFleet,startWindCapacForCFs,startSolarCapacForCFs,desiredTz,
    ...
    if len(windUnits)>1: #have wind in zone
        (windCFs,windCfsDtHr,windCfsDtSubhr,ewdIdAndCapac) = getWindCFs(windUnits,windDir,
            startWindCapacForCFs,desiredTz,windGenDataYr,
    ...
    if len(solarUnits)>1:
        (solarCFs,solarCfsDtHr,solarCfsDtSubhr,solarFilenameAndCapac) = getSolarCFs(solarUnits,solarDir,
            startSolarCapacForCFs,desiredTz,fipsToZones,fipsToPolys,currZone)
    ...
    return (windCFs,windCfsDtHr,windCfsDtSubhr,ewdIdAndCapac,
            ...
            if avgFleetCfHr == []: 
                ...
                if allSiteCfsHourly == []:  
                    a,tempAllSiteCfsHourly,tempAllSiteCfsSubhourly,tempEwdIdAndCapac = getWindCFs(windUnits,
                            windDir,0,desiredTz,windGenDataYr,fipsToZones,fipsToPolys,currZone) 
...
def getWindOrSolarIdsInZonesDecreasingCF(metadata,capacInZone,cfCol,capacCol,
        siteNumberOrFileCol,startRECapacForCFs,fipsToZones,fipsToPolys,currZone,windOrSolar):
    idAndCapacs = [['Id','DatasetCapacity','FleetCapacity']]
    (cfs,capacs,siteNumbers) = getPlantInfoInZone(metadata,startRECapacForCFs,
            cfCol,capacCol,siteNumberOrFileCol,fipsToZones,fipsToPolys,currZone)
    currZoneCapac = 0
    while currZoneCapac < capacInZone:
        if len(cfs)==0:
            ...
            if cfs[maxCfIdx]>1E-3: #some solar sites have 0 CF!
                if windOrSolar == 'Wind': datasetCapac = 70 #avg wind fleet capac in US (NEEDS v5.15, 860 2015)
                else: datasetCapac = 12 #avg solar capac in US (NEEDS v5.15, 860 2015)
                ...
                if currZoneCapac<startRECapacForCFs and currZoneCapac+fleetCapac>=startRECapacForCFs:
...
def getPlantInfoInZone(metadata,startRECapacForCFs,cfCol,capacCol,siteNumberOrFileCol,
    ...
    if 'longitude' in metadata[0]:  #wind has lat/long already given
        ...
        plantsInRegionOrZone = [row for row in metadata[1:] if locInZone(float(getCoordsFromFilename(row[nameCol])[0]),
                float(getCoordsFromFilename(row[nameCol])[1]),currZone,fipsToZones,fipsToPolys)]
...
def getSolarCFs(solarUnits,solarDir,startSolarCapacForCFs,
                desiredTz,fipsToZones,fipsToPolys,currZone):
    ...
    (solarFilenameAndCapac,solarFilenameAndCapacAndTz,solarMetadata) = getBestSolarIdsInStates(
                            solarDir,solarCapacInZone,startSolarCapacForCFs,
                            fipsToZones,fipsToPolys,currZone)
    #Import CFs for each wind plant
            ...
            if avgFleetCfHr == []: 
                ...
                if allSiteCfsHourly == []:  
                    a,tempAllSiteCfsHr,tempAllSiteCfsSubhr,tempFileAndCapac = getSolarCFs(solarUnits,solarDir,
                            0,desiredTz,fipsToZones,fipsToPolys,currZone) 
...
def getBestSolarIdsInStates(solarDir,solarCapacByState,startSolarCapacForCFs,
    ...
    idAndCapacs = getWindOrSolarIdsInZonesDecreasingCF(solarMetadata,solarCapacByState,
        solarCfCol,solarCapacCol,solarFilenameCol,startSolarCapacForCFs,fipsToZones,
...
def timezoneOfSolarSite(solarFilename,currZone):
    ...
    if currZone == 'S_C_TVA' or currZone == 'S_C_KY' or currZone == 'S_SOU':
        if currZone == 'S_C_TVA': line = tnLine
        elif currZone == 'S_C_KY': line = kyLine
        elif currZone == 'S_SOU': line = gaAlBorder
        (siteLat,siteLong) = getCoordsFromFilename(solarFilename)
        if siteEastOfLine(line,float(siteLat),float(siteLong)): tz = 'EST'
        else: tz = 'CST'
    elif currZone == 'S_VACA': tz = 'EST'
    return tz
...
def slimSolarGenToYear(gen,desiredTz,siteFilename):
    ...
    if missingVals > 0:
        genInYr = [gen[0]] + [[datetime.datetime(currYear,1,1,0,0)+datetime.timedelta(minutes=timeStepMins*ctr),
                    gen[1][genCol]] for ctr in range(missingVals)] + [row for row in gen[1:] if row[dtCol].year == currYear]
    else:
