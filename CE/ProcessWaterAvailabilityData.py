...
def setParams():
    ...
    baseDir = '/Volumes/KIKO64/Data/'
    dataDir = os.path.join(baseDir, 'DatafromUW', 'RBMRawWaterAvailabilityData19Aug2016')
    ...
    tgtYr = 2015
    # Fleet params
    genFleetDir = '/Users/kiko/Documents/CE/AreaTVACellsallCurtailEnvRegsCnoneSnor/CE'
    genFleetName = 'genFleetForCE2025.csv'
...
def loadNetworkData(dataDir, networkFilename):
    ...
    while 1:
        ...
        if 'Node' in lineSplit:  # if not Node row, skip it
            ...
            if (lat, lon) in mapLatLongToNode:
                mapLatLongToNode[(lat, lon)].append(node)
            else:
                mapLatLongToNode[(lat, lon)] = [node]
    f.close()
