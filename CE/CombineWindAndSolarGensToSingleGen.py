...
def combineWindAndSolarToSinglePlant(fleetUC,ipmZones,runLoc):
    for zone in ipmZones:
        combineWindOrSolarPlants(fleetUC,zone,runLoc,'Wind','Wind')
        combineWindOrSolarPlants(fleetUC,zone,runLoc,'Solar','Solar PV')

#Adds new combined unit, then removes other units
#Inputs: gen fleet (2d list), fuel type to combine, plant type to combine
def combineWindOrSolarPlants(fleetUC,zone,runLoc,fuelType,plantType):
    ...
    if len(rowIdxs)>0:
        ...
        addParametersToNewWindOrSolarRow(fleetUC,newRow,rowIdxs,fuelType,plantType,zone,runLoc)
...
#combined.
def addParametersToNewWindOrSolarRow(fleetUC,newRow,rowIdxs,fuelType,plantType,zone,runLoc):
