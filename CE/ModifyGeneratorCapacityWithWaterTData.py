...
def determineHrlyCurtailmentsForExistingGens(locPrecision,genFleet,rbmOutputDir,curtailmentYear,
        modelName,ptCurtailed,ptCurtailedRegs,incCurtailments,incRegs,envRegMaxT,runLoc,
    ...
    hrlyCurtailmentsAllGensInTgtYr,hrlyCurtailmentsList = calculateGeneratorCurtailments(cellLatLongToGenDict,
        rbmOutputDir,locPrecision,curtailmentYear,genFleet,modelName,ptCurtailed,ptCurtailedRegs,
...
################################################################################
#Returns 2 dicts for all gens in fleet that have lat/long coords. 
#1)gen:[gen,(gen lat, gen long),(cell lat, cell long)]. 2) (cell lat, clell long):genID.
#& list of [[genID,(genlat,genlong),(celllat,celllong)]]
...
################################################################################
#Calculates generator curtailments. If generator isn't curtailed (not right plant type,
#cell data not avaialble, etc.), ignored here. CalculateHourlyCapacs... script
#handles those cases (assume curtailment = 0).
#cellLatLongToGenDict = dict of (cell lat, cell long):gen ID for all gens w/ lat/lon coords
#in genFleet, including gens that hsould nto be curtailed. 
def calculateGeneratorCurtailments(cellLatLongToGenDict,rbmOutputDir,locPrecision,
            ...
            incRegs,envRegMaxT,runLoc,coolDesignT,resultsDir):
    hrlyCurtailmentsAllGensInTgtYr,hrlyCurtailmentsList = dict(),list()
    regCoeffs = loadRegCoeffs(runLoc) #dict of plant type: cooling type: cool design T: param: coeffs
            ...
            for gen in gensInCell:
                ...
                if coeffs != None or plantType in ptCurtailedRegs: #skip gens that aren't curtailed
                    hrlyCurtailmentsGen = calcCurtailmentForGenOrTech(plantType,hr,fuelAndCoalType,
                                coolType,fgdType,state,capac,ptCurtailed,ptCurtailedRegs,metAndWaterData,
                                incCurtailments,incRegs,envRegMaxT,coolDesignT,coeffs)
                    hrlyCurtailmentsAllGensInTgtYr[gen] = hrlyCurtailmentsGen
                    hrlyCurtailmentsList.append([gen] + hrlyCurtailmentsGen)
        else: print('Cell not in folders!:',cellFoldername)
...
#This should load cell-specific met data, but don't have that currently, so just 
#load regional data.
def loadMetData(cellLat,cellLong,rbmOutputDir,locPrecision,curtailmentYear):
    metData = pd.read_csv('C:\\Users\\mtcraig\\Desktop\\EPP Research\\PythonRIPSProject\\AreaTVACellsallNoCurtailCnoneSnor\\' + 
