...
def getNetDemand(hourlyDemand,windCFs,ewdIdAndCapac,solarCFs,solarFilenameAndCapac,currYear,
    ...
    if windCFs == None: hourlyWindGen = [0 for val in range(len(hourlyDemand))]
    else: hourlyWindGen = getHourlyGenProfile(windCFs,ewdIdAndCapac)
    if solarCFs == None: hourlySolarGen = [0 for val in range(len(hourlyDemand))]
    else: hourlySolarGen = getHourlyGenProfile(solarCFs,solarFilenameAndCapac)
