...
def forecastZonalDemandWithReg(yr,runLoc,ipmZones,resultsDir):
    zonalDemand,zonalTempDfs = dict(),dict()
    for zone in ipmZones:
        data,tempCoefs,intCoefs,fixEffHr,fixEffYr,intercept,holidays = loadRegData(runLoc)
...
def loadRegData(runLoc):
    if runLoc == 'pc': dataDir = 'C:\\Users\\mtcraig\\Desktop\\EPP Research\\Databases\\DemandDataFrancisco'
    else: dataDir = 'DemandDataFrancisco'
...
def getFixEffYrVal(yr,fixEffYr,dataYr):
    if yr in fixEffYr['year'].values: val = fixEffYr.loc[fixEffYr['year']==yr,'value']
    else: val = 0
