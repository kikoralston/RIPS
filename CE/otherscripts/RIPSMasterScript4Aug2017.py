...
from ModifyGeneratorCapacityWithWaterTData import (determineHrlyCurtailmentsForExistingGens,
        getGenToCellAndCellToGenDictionaries,createBaseFilenameToReadOrWrite,getCellLatAndLongFromFolderName,
...
def setKeyParameters():
    #KEY PARAMETERS
    runLoc = 'pc' #'pc' or 'remote'. If remote, assumes all data is in local dir. 
    ...
    (startYear,endYear,yearStepCE) = (2015,2046,10) 
    firstUCYear = 2015
    daysPerSeason = 10
    ...
    #RESULTS DIRECTORY
    resultsDir = 'C:\\Users\\mtcraig\\Desktop\\EPP Research\\PythonRIPSProject'
    ...
    if analysisArea == 'coreSERC':
        ...
        ipmZones = ['S_C_TVA'] 
    fipsToZones,fipsToPolys = getIPMPolys(runLoc,ipmZones) #get IPM polygons
    statePolys = getStatePolys(runLoc,states) #get state polygons
    ...
    write2dListToCSV([['Zone','ZoneNum']]+rotate([ipmZones,ipmZoneNums]),os.path.join(resultsDir,'zoneNamesToNumbers.csv'))
    #IMPORT TRANSMISSION SYSTEM DATA
    lineList,lineCapacs = setLines(ipmZones)
    #OLD VARIABLES
    testModel = False #use dummy test system; not currently working
    return (runLoc,xsedeRun,runCE,runUC,resultsDir,startYear,endYear,yearStepCE,daysPerSeason,
        ...
        scenario,incITC,ipmZoneNums,fipsToZones,fipsToPolys,statePolys,permitOncethru,incCurtailments,incRegs,
        phEff,phMaxSoc,phInitSoc,runFirstUCYear,coolDesignT)
...
def setThermalCurtailmentParameters():
    ...
    #Water T directories
    rbmRootDir = 'C:\\Users\\mtcraig\\Desktop\\EPP Research\\UWData\\DataFromUW\\DummyDataWithScripts9Aug2016'
...
################################################################################
def masterFunction():
    (runLoc,xsedeRun,runCE,runUC,resultsDir,startYear,endYear,yearStepCE,daysPerSeason,
        testModel,cellsEligibleForNewPlants,compressFleet,co2CapEndYr,co2CapEnd,
        fuelPricesTimeSeries,processRBMData,tzAnalysis,projectName,windGenDataYr,capacExpFilename,
        retirementCFCutoff,ptEligRetCF,daysPerSeason,selectCurtailDays,planningReserve,
        discountRate,allowCoalWithoutCCS,onlyNSPSUnits,ucFilename,calculateCO2Price,
    ...
    genFleet = getInitialFleetAndDemand(testModel,states,ipmZones,endYear,startYear,
        ...
        regUpCostCoeffs,ptCurtailed|ptCurtailedRegs) #combine curtailed sets
    print('Set up initial data')    
    if processRBMData == True: processRBMDataToUsableFormat()
    ...
    for currYear in range(startYear,endYear,yearStepCE):
        ...
        zonalDemandProfile,zonalTempDfs = forecastZonalDemandWithReg(currYear,runLoc,ipmZones,resultsDir)
        if currYear > firstUCYear and runCE == True:
            ...
            (genFleet,genFleetNoRetiredUnits,genFleetPriorCE,priorCapacExpModel,
                    priorHoursCE) = runCapacityExpansion(genFleet,zonalDemandProfile,
                    startYear,currYear,endYear,planningReserve,discountRate,
                    ...
                    ocAdderMin,ocAdderMax,maxAddedZonalCapacPerTech,windGenDataYr,
                    regUpCostCoeffs,xsedeRun,scenario,ptEligRetCF,
                    genFleetPriorCE,priorCapacExpModel,priorHoursCE,incITC,
                    selectCurtailDays,cellsEligibleForNewPlants,ipmZones,
                    lineList,lineCapacs,cellNewTechCriteria,ptCurtailed,ptCurtailedRegs,
                    ipmZoneNums,fipsToZones,fipsToPolys,statePolys,permitOncethru,incCurtailments,incRegs,
        ...
        if runUC == True:
            ...
            if ((currYear == firstUCYear and runFirstUCYear == True) or 
                ...
                (ucResultsByDay,hourlyGenerationByPlants) = runUnitCommitment(genFleetNoRetiredUnits,
                        ...
                        ocAdderMin,ocAdderMax,windGenDataYr,regLoadFrac,contLoadFrac,
                        regErrorPercentile,flexErrorPercentile,rrToRegTime,rrToFlexTime,rrToContTime,
                        copy.deepcopy(regUpCostCoeffs),xsedeRun,runCE,scenario,ucFilename,ipmZones,incCurtailments)
################################################################################
################################################################################
################################################################################
...
def getInitialFleetAndDemand(testModel,states,ipmZones,endYear,
        startYear,fuelPricesTimeSeries,compressFleet,runLoc,resultsDir,ocAdderMin,
...
def runCapacityExpansion(genFleet,zonalDemandProfile,startYear,currYear,endYear,
    ...
    newTechsCE = getNewTechs(allowCoalWithoutCCS,onlyNSPSUnits,regUpCostCoeffs,currYear,
                            runLoc,resultsDir,scenario,incITC,permitOncethru)
    ...
    combineWindAndSolarToSinglePlant(genFleetForCE,ipmZones,runLoc) #combines by zone
    ...
    for zone in ipmZones:
        ...
        (windCFs,windCfsDtHr,windCfsDtSubhr,ewdIdAndCapac,solarCFs,solarCfsDtHr,solarCfsDtSubhr,
            ...
            tzAnalysis,runLoc,windGenDataYr,zone,fipsToZones,fipsToPolys)
        print('Got RE CFs')
        if windCFs != None:
            ...
            write2dListToCSV(ewdIdAndCapac,os.path.join(resultsDir,'windIdAndCapacCE' + zone + str(currYear) + '.csv'))
        if solarCFs != None:
            ...
            write2dListToCSV(solarFilenameAndCapac,os.path.join(resultsDir,'solarIdAndCapacCE' + zone + str(currYear) + '.csv'))
        (newWindCFs,newWindCFsSubhr,newSolarCFs,newSolarCFsSubhr,newWindCfsDtHr,newWindCfsDtSubhr,
            ...
            currYear,'CE',tzAnalysis,runLoc,resultsDir,windGenDataYr,zone,fipsToZones,fipsToPolys)
        print('Got new RE CFs')
        write2dListToCSV(newWindIdAndCapac,os.path.join(resultsDir,'windNewIdAndCapacCE' + zone + str(currYear) + '.csv'))
        write2dListToCSV(newSolarFilenameAndCapac,os.path.join(resultsDir,'solarNewIdAndCapacCE' + zone + str(currYear) + '.csv'))
    ...
    hrlyCurtailmentsAllGensInTgtYr = importHourlyThermalCurtailments(genFleetForCE, #existing gens only!
            currYear,'CE',ptCurtailed,ptCurtailedRegs,resultsDir,incCurtailments,incRegs,
            envRegMaxT,runLoc,coolDesignT)
    (demandCE,hoursForCE,repHrsBySeason,specialHrs,regHrsBySeason,demandCEZonal,
            ...
            zonalNetDemand,zonalHourlyWindGen,zonalHourlySolarGen,daysPerSeason,selectCurtailDays,
            hrlyCurtailmentsAllGensInTgtYr,currYear,resultsDir,planningReserve)
    ...
    hydroPotPerSeason = getHydroEPotential(genFleetForCE,zonalDemandProfile,repAndSpeHoursDict,currYear,runLoc)
    for season in hydroPotPerSeason: 
        writeDictToCSV(hydroPotPerSeason[season],os.path.join(resultsDir,'hydroPot' + season + 'CE' + str(currYear) + '.csv'))
    #Handle curtailments 
    ...
    hrlyCurtailmentsAllTechsInTgtYr,hrlyTechCurtailmentsList = determineHrlyCurtailmentsForNewTechs(eligibleCellWaterTs,
            newTechsCE,currYear,cellNewTechCriteria,ptCurtailed,ptCurtailedRegs,fipsToZones,fipsToPolys,
            incCurtailments,incRegs,rbmOutputDir,locPrecision,statePolys,runLoc,coolDesignT,
            envRegMaxT,resultsDir) 
    write2dListToCSV(hrlyTechCurtailmentsList,os.path.join(resultsDir,'curtailmentsTechHourly'+str(currYear)+'.csv'))
    (hourlyCurtailedTechCapacsCE,curtailedTechHourlyCapacsCEList,
            ...
            hrlyCurtailmentsAllTechsInTgtYr,hoursForCE,currYear,ptCurtailed|ptCurtailedRegs)     
    write2dListToCSV(curtailedTechHourlyCapacsList,os.path.join(resultsDir,'curtailedTechHourlyCapacs' + str(currYear) + '.csv'))
    write2dListToCSV(curtailedTechHourlyCapacsCEList,os.path.join(resultsDir,'curtailedTechHourlyCapacsCE' + str(currYear) + '.csv'))
    ...
    (capacExpModel,ms,ss,curtailedTechs,renewTechs,notCurtailedTechs,
        ...
        hourlyWindGenCEZonal,hourlySolarGenCEZonal,demandCEZonal,newTechsCE,planningReserveZonal,discountRate,
        hoursForCE,newWindCFsCEZonal,newSolarCFsCEZonal,scaleMWtoGW,scaleDollarsToThousands,currCo2Cap,
        capacExpFilename,seasonDemandWeights,repHrsBySeason,specialHrs,peakDemandHourZonal,
        scaleLbToShortTon,runLoc,maxAddedZonalCapacPerTech,xsedeRun,ipmZones,lineList,lineCapacs,
        cellsToZones,ipmZoneNums,ptCurtailed|ptCurtailedRegs,hydroPotPerSeason,phEff,phMaxSoc,phInitSoc)
    ...
    (genByPlant,genByCTechAndCell,genByRETechAndZone,genByNCTechAndZone,sysResults,
            ...
            genFleetForCE,curtailedTechs,renewTechs,notCurtailedTechs,cellsForNewTechs,ipmZoneNums,
            hoursForCE,lineList,pumpedHydroSymbs)
    ...
    genFleet = addNewGensToFleet(genFleet,newCurtTech,newRETech,newNotCurtTech,newTechsCE,
                currYear,ipmZones,ipmZoneNums,ocAdderMin,ocAdderMax,cellsToZones,ptCurtailed|ptCurtailedRegs,
                statePolys)
    genFleet = selectAndMarkUnitsRetiredByCE(genFleet,genFleetForCE,retirementCFCutoff,capacExpModel,currYear,
                ...
                planningReserveZonal,endYear,capacExpRetiredUnitsByAge,demandCEZonal,hourlyWindGenCEZonal,
                hourlySolarGenCEZonal,newWindCFsCEZonal,newSolarCFsCEZonal,ptEligRetCF)
...
def importHourlyThermalCurtailments(genFleet,currYear,modelName,ptCurtailed,ptCurtailedRegs,
                    resultsDir,incCurtailments,incRegs,envRegMaxT,runLoc,coolDesignT):
    (outputHeaders,locPrecision,tempAndSpatFilename,nsegFilename,
            rbmRootDir,rbmDataDir,rbmOutputDir,numCellsToProcess,envRegMaxT) = setThermalCurtailmentParameters()
    (hrlyCurtailmentsAllGensInTgtYr,genToCellLatLongsList,
            ...
            genFleet,rbmOutputDir,currYear,modelName,ptCurtailed,ptCurtailedRegs,
            incCurtailments,incRegs,envRegMaxT,runLoc,coolDesignT,resultsDir)
...
def callCapacityExpansion(genFleetForCE,hourlyCapacsCE,hourlyCurtailedTechCapacsCE,
        ...
        hoursForCE,newWindCFsCEZonal,newSolarCFsCEZonal,scaleMWtoGW,scaleDollarsToThousands,currCo2Cap,
        capacExpFilename,seasonDemandWeights,repHrsBySeason,specialHrs,peakDemandHourZonal,
        scaleLbToShortTon,runLoc,maxAddedZonalCapacPerTech,xsedeRun,ipmZones,
        lineList,lineCapacs,cellsToZones,ipmZoneNums,ptCurtailedAll,hydroPotPerSeason,
    ...
    (genSet,genSymbols,hourSet,hourSymbols,techSet,techSymbols,techCurtailedSet,techCurtailedSymbols,
        ...
        hoursForCE,newTechsCE,repHrsBySeason,specialHrs,peakDemandHourZonal,ipmZoneNums,
    ...
    addParametersToDatabaseCE(db,hourlyCapacsCE,hourlyWindGenCEZonal,hourlySolarGenCEZonal,demandCEZonal,
        newTechsCE,genFleetForCE,genSet,genSymbols,hydroGenSet,hydroGenSymbols,hourSet,hourSymbols,techSet,
        techSymbols,techCurtailedSet,techCurtailedSymbols,renewTechSet,renewTechSymbols,planningReserveZonal,
        discountRate,newWindCFsCEZonal,newSolarCFsCEZonal,scaleMWtoGW,scaleDollarsToThousands,
        currCo2Cap,seasonDemandWeights,hourlyCurtailedTechCapacsCE,scaleLbToShortTon,maxAddedZonalCapacPerTech,
        zoneSet,zoneSymbols,lineSet,ipmZones,ipmZoneNums,lineList,lineCapacs,cellsToZones,
        cellSet,peakDemandHourZonal,peakHourSet,peakHrSymbols,hydroPotPerSeason,ptCurtailedAll,
    ...
    if int(ms) != 8 or int(ms) != 1 or int(ss) != 1: print('Modelstat & solvestat:',ms,' & ',ss,
...
def addParametersToDatabaseCE(db,hourlyCapacsCE,hourlyWindGenCEZonal,hourlySolarGenCEZonal,demandCEZonal,
    ...
    addMaxNumNewBuilds(db,newTechsCE,zoneSet,ipmZones,ipmZoneNums,techSet,maxAddedZonalCapacPerTech,ptCurtailedAll)
    ...
    addTechCurtailedHourlyCapac(db,hourlyCurtailedTechCapacsCE,cellSet,techCurtailedSet,hourSet,hourSymbols,scaleMWtoGW)
...
def runUnitCommitment(genFleet,demandScaled,startYear,ucYear,fuelPricesTimeSeries,
        ...
        regErrorPercentile,flexErrorPercentile,rrToRegTime,rrToFlexTime,rrToContTime,
        regUpCostCoeffsUC,xsedeRun,runCE,scenario,ucFilename,incCurtailments):
    ...
    (windCFs,windCfsDtHr,windCfsDtSubhr,windIdAndCapac,solarCFs,solarCfsDtHr,solarCfsDtSubhr,
            ...
            states,statesAbbrev,tzAnalysis,projectName,runLoc,windGenDataYr)
    ...
    for dayIdx in range(0,len(daysForUC),daysOpt):
        ...
        ucModel,ms,ss = callUnitCommitment(fleetUC,ucFilename,hourlyCapacsUC,hourlyWindGenUC,
            ...
            regDownUC,runLoc,rrToRegTime,rrToFlexTime,rrToContTime,flexUC,contUC,xsedeRun)
...
def callUnitCommitment(fleetUC,ucFilename,hourlyCapacsUC,hourlyWindGenUC,
    ...
    addParametersToDatabaseUC(dbUC,hourlyCapacsUC,hourlyWindGenUC,hourlySolarGenUC,demandUC,fleetUC,
        genSet,genSymbols,hydroGenSet,hydroGenSymbols,hourSet,hourSymbols,cnse,rrToFlexTime,
