...
def master():
    ...
    if 'nsrdbMetadata.csv' in [f for f in os.listdir(outputDir) if os.path.isfile(os.path.join(outputDir, f))]:
        ...
        elevations = [['queriedLat', 'queriedLon', 'nsrdbLat', 'nsrdbLon', 'elevation(m)']]
...
def downloadLatLonNSRDBAPI(lat, lon, years, outputDir, whoseApiKey):
    ...
    if whoseApiKey == 'michael':
        with open(os.path.join('nsrdbApiKey.txt')) as f:
            api_key = f.read()
        your_name = 'Michael+Craig'
        your_email = 'mtcraig@andrew.cmu.edu'
    elif whoseApiKey == 'paulina':
        with open(os.path.join('nsrdbApiKeyPauli.txt')) as f:
            api_key = f.read()
    ...
    for idx in range(len(years)):
        ...
        if idx == 0:
            ...
            dfAllYears = pd.concat([dfAllYears, df], ignore_index=True)
        time.sleep(5)
