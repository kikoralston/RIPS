...
def selectWeeksForExpansion(zonalDemandProfile,zonalNetDemand,zonalHourlyWindGen,
    ...
    if selectCurtailDays == True: #if considering thermal curtailments in selection of special days
        ...
        peakNetDemandAndCurtailmentDayHours = getPeakDemandPlusCurtailmentDayHours(netDemand,totalHrlyCurtailments) #1-8760
    ...
    (demandCEZonal,hourlyWindGenCEZonal,hourlySolarGenCEZonal) = isolateDemandAndREGenForCEZonal(hoursForCE,
                                                    zonalDemandProfile,zonalHourlyWindGen,zonalHourlySolarGen)
...
def sumZonalData(zonalData):
    ...
    for zone in zonalData: 
        if totalData == []: totalData = copy.deepcopy(zonalData[zone])
        else: totalData = list(map(operator.add,totalData,zonalData[zone]))
    return totalData
...
def getTotalSystemCurtailments(hrlyCurtailmentsAllGensInTgtYr): 
    ...
    for gen in hrlyCurtailmentsAllGensInTgtYr:
        ...
        if totalHrlyCurtailments==None: totalHrlyCurtailments = copy.deepcopy(genHrlyCurtailments)
        else: totalHrlyCurtailments = list(map(operator.add,totalHrlyCurtailments,genHrlyCurtailments))
...
def isolateDemandAndREGenForCEZonal(hoursForCE,zonalDemandProfile,zonalHourlyWindGen,zonalHourlySolarGen):
    ...
    for zone in zonalDemandProfile:
        demandCEZonal[zone] = [zonalDemandProfile[zone][hr-1] for hr in hoursForCE] #-1 b/c hours in year start @ 1, not 0 like Python idx
        hourlyWindGenCEZonal[zone] = [zonalHourlyWindGen[zone][hr-1] for hr in hoursForCE] #-1 b/c hours in year start @ 1, not 0 like Python idx
        hourlySolarGenCEZonal[zone] = [zonalHourlySolarGen[zone][hr-1] for hr in hoursForCE] #-1 b/c hours in year start @ 1, not 0 like Python idx    
