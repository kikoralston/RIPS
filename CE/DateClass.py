...
class Date(object):
    ...
    def __init__(self,year,month,day,hour,timezone):
        ...
        self.day = int(day)
        if type(hour) == int: self.hour = hour
        else:
            if ':' not in hour: self.hour = int(hour)
            else: self.hour = int(hour[:hour.index(':')]) + 1 #add 1 to convert to 1-24
    ...
    def convertToTimezone(self,tgtTimezone):
        ...
        if tgtTimezone=='EST' and self.timezone=='UTC': timeChange = -4
        elif tgtTimezone=='EST' and self.timezone=='CST': timeChange = 1
        elif tgtTimezone=='EST' and self.timezone=='EST': timeChange = 0
        if timeChange != 0:
            if timeChange < 0:
                if hour > abs(timeChange): hour += timeChange 
                else:
                ...
                if hour + timeChange <= 24: hour += timeChange
                else:
                    hour = hour - 24 + timeChange
                    if day < self.daysPerMonth[month-1]: day += 1
                    else:
                        day = 1
                        if month < 12: month += 1
                        else: 
