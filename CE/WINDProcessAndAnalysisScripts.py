...
################ COPY SUBHOURLY SERC WIND DATA #################################
def copySERCSubhourlyData():
    print('Copying subhourly data')
    sercMetadata = readCSVto2dList('E:\\toolkit_sites_v7_SERC.csv')
    sampleDts,hourlyDts = createDts()
    siteCol = sercMetadata[0].index('site_id')
    for site in sercMetadata[1:]:
        siteId = int(site[siteCol])
        lat,lon,power = extractCDFData(os.path.join('E:','met_data',str(siteId//500),str(siteId)+'.nc'))
        powerSubhourly2D = [['Datetime',siteId]]
        for idx in range(len(power)): powerSubhourly2D.append([sampleDts[idx],power[idx]])
        write2dListToCSV(powerSubhourly2D,os.path.join('E:','subhourlyPowerSERC','powersubhourly_' + str(siteId) + '.csv'))
        print('Finished ' + str(siteId))

copySERCSubhourlyData()
################################################################################
