...
def determineHrlyCurtailmentsForNewTechs(eligibleCellWaterTs,newTechsCE,currYear,cellNewTechCriteria,
            ptCurtailed,ptCurtailedRegs,fipsToZones,fipsToPolys,incCurtailments,incRegs,rbmOutputDir,
            locPrecision,statePolys,runLoc,coolDesignT,envRegMaxT,resultsDir):
    ...
    regCoeffs = loadRegCoeffs(runLoc) #dict of cooling type: reg coeffs
        ...
        for cell in cellWaterTsForNewTechs:
            ...
            if coeffs != None or plantType in ptCurtailedRegs: #skip gens that aren't curtailed
                hrlyCurtailmentsGen = calcCurtailmentForGenOrTech(plantType,hr,fuelAndCoalType,
                        coolType,fgdType,state,ptCurtailed,ptCurtailedRegs,metAndWaterData,
                        incCurtailments,incRegs,envRegMaxT,coolDesignT,coeffs)
                hrlyCurtailmentsAllTechsInTgtYr[(createTechSymbol(techRow,newTechsCE[0],ptCurtailed|ptCurtailedRegs),
                                            cell)] = hrlyCurtailmentsGen
                hrlyTechCurtailmentsList.append([createTechSymbol(techRow,newTechsCE[0],ptCurtailed|ptCurtailedRegs),cell] 
                                                                                        + hrlyCurtailmentsGen)
...
def getCellsInEachZone(eligibleCellWaterTsCurrYear,fipsToZones,fipsToPolys):
    ...
    for cell in eligibleCellWaterTsCurrYear:
        ...
        if cellZone in cellsPerZone: cellsPerZone[cellZone].append(cell)
        else: cellsPerZone[cellZone] = [cell]
    return cellsPerZone
