...
def setKeyParameters():
    ...
    newTechFrameworkCsv = 'NewTechFramework.csv'
...
def getNewPlantATBData():
    (plantCsvNames,atbDir,paramAtbNames,newTechFramework,csvToTechType,paramAtbScalar) = setKeyParameters()    
...
#of compiled results.
def compileNewTechData(atbDir,plantCsvNames,paramAtbNames,newTechFramework,csvToTechType,paramAtbScalar):
    newTechData = []
    ...
    for plantCSV in plantCsvNames:
        if type(csvToTechType[plantCSV])==tuple:
            ...
            plantData = readDataFromCSV(atbDir,plantCSV,newTechFramework,fleetTechType,paramAtbNames,paramAtbScalar)
            newTechData += plantData
    write2dListToCSV(newTechData,os.path.join(atbDir,'newTechDataATB.csv'))
    return newTechData
...
def readDataFromCSV(atbDir,plantCSV,newTechFramework,fleetTechType,paramAtbNames,paramAtbScalar):
