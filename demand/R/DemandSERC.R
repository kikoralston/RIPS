...
read.total.demand.serc <- function() {
  ...
  return(df.ferc.final)
}


compile.meteodata.serc <- function(yearini=2006, yearend=2015) {
  path.demand <- '~/Documents/CE/Data/DemandData/'
  
  df.zones <- read.csv(paste0(path.demand, 'map_zone_cell.csv'))
  
  zones <- unique(df.zones$zone)
  ...
  for (z in zones) {
    ...
    for (i in 1:nrow(aux)) {
      ...
      if (i == 1){
        df.temp <- read.csv(paste0(path.demand, '/training/', fname))
        df.temp$city <- aux$X[i]
      } else {
        df.temp.1 <- read.csv(paste0(path.demand, '/training/', fname))
    ...
    df.final <- df.temp %>% mutate(zone=z)
    
    #year.end <- max(year(df.ferc.final$time))
    #year.ini <- min(year(df.ferc.final$time))
    
    df.final <- df.final %>% 
      transmute(time=time, zone=zone, temp=air_T, rh=rel_humid, 
                dp=convertRelHum2DewPoint(rh, temp)) %>% as.data.frame()
    ...
    df.final <- df.final %>% filter(year >= yearini & year <= yearend)
    
    if (is.null(df.meteo.serc)) {
      df.meteo.serc <- df.final
    } else {
      df.meteo.serc <- rbind(df.meteo.serc,
                             df.final)
    }
  }
  
  return(df.meteo.serc)
  
}

run.regression.zone.serc <- function(df.total, z) {
  df.final <- df.total %>% filter(year <= 2015)
