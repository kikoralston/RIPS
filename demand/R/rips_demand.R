...
source(paste0(path.rips, "/demand/R/downloadNOAA.R"))
...
source(paste0(path.rips, "/demand/R/BLS_Data.R"))


# functions to fir linear regression models and export parameters ----

fit.lm.model <- function(df, 
  ...
  #
  # Args:
  #   df:  data frame with data set to be used in regression
...
compute.calendar.variables <- function(df) {
  ...
  year.end <- max(lubridate::year(df$time))
  year.ini <- min(lubridate::year(df$time))
  ...
  # set holidays
  library("timeDate")
  holi.days <- date(timeDate::holidayNERC(year = c(year.ini:year.end)))
...
# functions to prepare data and read data frame  ------------------------------
...
readAllData <- function(save.to.file = TRUE, name.of.file = "",
  ...
  df.final <- cbind(data.frame(year=lubridate::year(df.final$time)), df.final)
...
# functions to perform projections  ------------------------------
...
#    summarize(mean.value = mean(y.hat, na.rm=TRUE))
  ...
  load.proj <- load.proj %>% select(time, hour.of.day, season, y.hat)
...
create.base.line <- function(name.file='data_memphis_1973.rds', 
  ...
  base.line.data <- df %>%
    dplyr::filter(lubridate::year(time) >= period[1] & lubridate::year(time) <= period[2])
...
create.projection.weather <- function(url.uw="~/GoogleDrive/CMU/RIPS/UW/meteo_memphis",
  ...
  if (type == 'original') {
    uw.temp <- uw.data %>% dplyr::transmute(time=time, value=temp)
    uw.dp <- uw.data %>% dplyr::transmute(time=time, value=dp)
    base.line.temp <- base.line.data %>% dplyr::transmute(time=time, value=temp)
    base.line.dp <- base.line.data %>% dplyr::transmute(time=time, value=dp)
...
project.demand.2 <- function(lm.hourly, 
                             ...
                             temp.breaks=c(0, 10, 20, 30),
                             weather.data) {
# Computes demand forecast using the simple transformation of the weather data suggested by aufhammer.
#
